/*
 * $Id: cdk_device.h,v 1.2 Broadcom SDK $
 *
...
typedef struct cdk_dev_s {
    ...
    /* Callback funciton for sbus mdio read/write PHY data */
    int (*cdk_sbus_mido_read)(int unit, uint32_t addr, uint32_t reg, uint32_t *data);
    int (*cdk_sbus_mido_write)(int unit, uint32_t addr, uint32_t reg, uint32_t data);
} cdk_dev_t; 
