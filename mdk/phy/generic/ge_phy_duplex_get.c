/*
 * $Id: ge_phy_duplex_get.c,v 1.3 Broadcom SDK $
 *
...
ge_phy_duplex_get(phy_ctrl_t *pc, int *duplex)
{
    ...
    if (ctrl & MII_CTRL_AE) {
        ...
        ioerr += PHY_BUS_READ(pc, 0x18, &misc_ctrl);

        /* First check for Ethernet@Wirespeed */
        if ( misc_ctrl & (0x1 << 4) ) {   /* Ethernet@Wirespeed enabled */
            return ge_phy_autoneg_ew(pc, NULL, duplex);
        } else {
