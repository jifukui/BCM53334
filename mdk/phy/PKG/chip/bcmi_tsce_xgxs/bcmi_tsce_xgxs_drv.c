/*
 * $Id: bcmi_tsce_xgxs_drv.c,v 1.14 Broadcom SDK $
 *
...
static unsigned long long init_speed = 0x0;
...
/*
 * Function:
 *      _tsce_abiliby_remote_get
 * Purpose:
 *      Get remote phy ability.
 * Parameters:
 *      pc - PHY control structure
 *      ability - ability value
 * Returns:
 ...
 */
static int
_tsce_abiliby_remote_get(phy_ctrl_t *pc, uint32_t *ability)
{
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
    phymod_autoneg_ability_t autoneg_ability; 
    ...
    _tsce_phymod_phy_t_init(pc, &pm_phy);
    rv = tsce_phy_autoneg_remote_ability_get(&pm_phy, &autoneg_ability);
    ...
    *ability |= PHY_ABIL_1000MB_FD ;
    ...
    /* retrieve "pause" abilities */
    reg_ability = autoneg_ability.capabilities;
    if (reg_ability & (PHYMOD_AN_CAP_SYMM_PAUSE|PHYMOD_AN_CAP_ASYM_PAUSE)) {
        *ability |= PHY_ABIL_PAUSE_RX;
    }
    if (reg_ability & PHYMOD_AN_CAP_ASYM_PAUSE) {
        *ability |= PHY_ABIL_PAUSE_TX;
    }
    if (reg_ability & PHYMOD_AN_CAP_SYMM_PAUSE) {
        *ability |= PHY_ABIL_PAUSE;
    }
...
bcmi_tsce_xgxs_init(phy_ctrl_t *pc)
{
    ...
    CDK_MEMSET(&interface_config, 0, sizeof(phymod_phy_inf_config_t));

    PHY_CONFIG_GET(pc, PhyConfig_InitSpeed, &initspeed, NULL);

    interface_config.data_rate = initspeed;
...
bcmi_tsce_xgxs_speed_set(phy_ctrl_t *pc, uint32_t speed)
{
    ...
    /* 10G KR mode is an=on, cl73=on, need to do speed_set */
    if (speed == 10000) {
        if (autoneg) {
            return CDK_E_NONE;
        }
...
static int
_tsce_register_get(phy_ctrl_t *pc, uint32_t addr, uint32_t *data) {
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
    ...
    rv = phymod_tsc_iblk_read(&pm_phy.access, addr, data);
    
    return rv;
}

static int
_tsce_register_set(phy_ctrl_t *pc, uint32_t addr, uint32_t data) {
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
    ...
    rv = _tsce_phymod_phy_t_init(pc, &pm_phy);
    rv = phymod_tsc_iblk_write(&pm_phy.access, addr, data);
...
bcmi_tsce_xgxs_config_set(phy_ctrl_t *pc, phy_config_t cfg, uint32_t val, void *cd)
{
    ...
    switch (cfg) {
    case PhyConfig_InitSpeed:
        if (val >= 10000) {
            init_speed |= (unsigned long long)1 << (pc->port);
        }
        return rv;
    ...
    case PhyConfig_MultiCoreLaneMap:
        ...
        return rv;
    case PhyConfig_RegisterAccess:
        addr = val;
        rv = _tsce_register_set(pc, addr, *(uint32_t *)cd);
        return rv;
    default:
        break;
    }
...
bcmi_tsce_xgxs_config_get(phy_ctrl_t *pc, phy_config_t cfg, uint32_t *val, void *cd)
{
    ...
    switch (cfg) {
    case PhyConfig_InitSpeed:
        if (init_speed & ((unsigned long long)1 << (pc->port))) {
            /* init speed = 10000 */
            *val = 10000;
        } else {
            *val = 1000;
    ...
    case PhyConfig_AdvRemote: 
        rv = _tsce_abiliby_remote_get(pc, val);
        return rv;
#endif
    case PhyConfig_EEE:
        {
    ...
    case PhyConfig_RegisterAccess:
        addr = *val;
        rv = _tsce_register_get(pc, addr, (uint32_t *)cd);
        return rv;
    default:
        break;
    }
