/*
 * $Id: bcmi_qsgmiie_serdes_drv.c,v 1.5 Broadcom SDK $
 *
...
static int
_qsgmiie_register_get(phy_ctrl_t *pc, uint32_t addr, uint32_t *data) {
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
    
    PHY_CTRL_CHECK(pc);

    rv = _qsgmiie_phymod_phy_init(pc, &pm_phy);
...
static int
_qsgmiie_register_set(phy_ctrl_t *pc, uint32_t addr, uint32_t data) {
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
...
bcmi_qsgmiie_serdes_config_set(phy_ctrl_t *pc, phy_config_t cfg, uint32_t val, void *cd)
{
    ...
    case PhyConfig_RegisterAccess:
        addr = val;
        rv = _qsgmiie_register_set(pc, addr, *(uint32_t *)cd);
        return rv;
    default:
        break;
    }
...
bcmi_qsgmiie_serdes_config_get(phy_ctrl_t *pc, phy_config_t cfg, uint32_t *val, void *cd)
{
    ...
    case PhyConfig_RegisterAccess:
        addr = *val;
        rv = _qsgmiie_register_get(pc, addr, (uint32_t *)cd);
        return rv;
    default:
        break;
    }
