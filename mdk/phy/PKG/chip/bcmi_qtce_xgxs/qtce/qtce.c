...
int qtce_core_info_get(const phymod_core_access_t* phy, phymod_core_info_t* info)
{
    ...
    if ((serdes_id & 0x3f) == QTCE4_MODEL) {
        ...
        PHYMOD_STRNCPY(info->name, "QTCe4A0", PHYMOD_STRLEN("Tsce4A0")+1); 
    } else {
...
static int _qtce_pll_multiplier_get(uint32_t pll_div, uint32_t *pll_multiplier)
{
    switch (pll_div) {
    case 0x0:
        *pll_multiplier = 46;
...
static int _qtce_speed_id_interface_config_get(const phymod_phy_access_t* phy, int speed_id, 
...
int qtce_phy_autoneg_ability_get(const phymod_phy_access_t* phy, phymod_autoneg_ability_t* an_ability_get_type){
    ...
    PHYMOD_IF_ERR_RETURN
        (qmod_autoneg_local_ability_get(&phy_copy.access, &value));
