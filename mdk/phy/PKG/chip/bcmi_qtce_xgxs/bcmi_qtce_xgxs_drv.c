/*
* $Id: bcmi_qtce_xgxs_drv.c,v 1.2 Broadcom SDK $
*
...
_qtce_phymod_core_init(phy_ctrl_t *pc, phymod_core_access_t *core)
{
    ...
    core->access.user_acc = (void *)pc;  /* Use user data pointer to keep whole phy ctrl data */
    core->access.bus = NULL;             /* Unused */
    core->access.addr = 0;               /* Unused */
    core->access.devad = 0;              /* Unused */
...
_qtce_phymod_phy_init(phy_ctrl_t *pc, phymod_phy_access_t *phy, uint32_t phyidx)
{
    ...
    phy->access.bus = NULL;             /* Unused */
    phy->access.addr = 0;               /* Unused */
    phy->access.devad = 0;              /* Unused */
...
bcmi_qtce_xgxs_init(phy_ctrl_t *pc)
{
    ...
    if (PHYMOD_ACC_F_QMODE_GET(&pm_phy.access)) {
        if (CDK_SUCCESS(rv)) {
            rv = qtce_phy_autoneg_ability_set(&pm_phy, &autoneg_ability);
        }
...
static int
_qtce_register_get(phy_ctrl_t *pc, uint32_t addr, uint32_t *data) {
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
...
static int
_qtce_register_set(phy_ctrl_t *pc, uint32_t addr, uint32_t data) {
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
...
bcmi_qtce_xgxs_config_set(phy_ctrl_t *pc, phy_config_t cfg, uint32_t val, void *cd)
{
    ...
    case PhyConfig_EEE:
        {
            for (idx = 0; idx < pc->num_phys; idx++) {
                ...
                _qtce_phymod_phy_init(lpc, &pm_phy, 0);
                if (CDK_SUCCESS(rv)) {
                    rv = qtce_phy_eee_set(&pm_phy, val);
                }
            }
    ...
    case PhyConfig_RegisterAccess:
        addr = val;
        rv = _qtce_register_set(pc, addr, *(uint32_t *)cd);
        return rv;
    default:
        break;
    }
...
bcmi_qtce_xgxs_config_get(phy_ctrl_t *pc, phy_config_t cfg, uint32_t *val, void *cd)
{
    ...
    case PhyConfig_EEE:
        {
            _qtce_phymod_phy_init(pc, &pm_phy, 0);
            if (CDK_SUCCESS(rv)) {
                rv = qtce_phy_eee_get(&pm_phy, val);
            }
            return rv;
        }
    case PhyConfig_TxPreemp:
    ...
    case PhyConfig_RegisterAccess:
        addr = *val;
        rv = _qtce_register_get(pc, addr, (uint32_t *)cd);
        return rv;
    default:
        break;
    }
