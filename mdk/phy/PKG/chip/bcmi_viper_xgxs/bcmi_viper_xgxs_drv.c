/*
 * $Id: bcmi_viper_xgxs_drv.c,v 1.3 Broadcom SDK $ Broadcom SDK $
...
_viper_ability_set(phy_ctrl_t *pc, uint32_t ability)
{
    ...
    PHYMOD_MEMSET(&autoneg_ability, 0x0, sizeof(autoneg_ability));
...
_viper_ability_get(phy_ctrl_t *pc, uint32_t *ability)
{
    ...
    PHYMOD_MEMSET(&phymod_autoneg_ability, 0x0, sizeof(phymod_autoneg_ability));
    ...
    *ability = 0;
    ...
    } else if (reg_ability & PHYMOD_AN_CAP_ASYM_PAUSE) {
 ...
 */
static int
_viper_ability_remote_get(phy_ctrl_t *pc, uint32_t *ability)
{
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
    phymod_autoneg_ability_t phymod_autoneg_ability;
    ...
    rv = viper_phy_autoneg_remote_ability_get(&pm_phy, &phymod_autoneg_ability);

    *ability = 0;
    
    /* retrieve CL37 abilities */
    /* reg37_ability = phymod_autoneg_ability.an_cap;
    *ability |= PHYMOD_AN_CAP_1G_KX_GET(reg37_ability)? SOC_PA_SPEED_1000MB : 0;
    *ability |= PHY_ABIL_1000MB; */
...
static int
_viper_register_get(phy_ctrl_t *pc, uint32_t addr, uint32_t *data) {
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
    
    PHY_CTRL_CHECK(pc);

    rv = _viper_phymod_phy_t_init(pc, &pm_phy);
...
static int
_viper_register_set(phy_ctrl_t *pc, uint32_t addr, uint32_t data) {
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
...
bcmi_viper_xgxs_config_set(phy_ctrl_t *pc, phy_config_t cfg, uint32_t val, void *cd)
{
    ...
    case PhyConfig_RegisterAccess:
        addr = val;
        rv = _viper_register_set(pc, addr, *(uint32_t *)cd);
        return rv;
    default:
        break;
    }
...
bcmi_viper_xgxs_config_get(phy_ctrl_t *pc, phy_config_t cfg, uint32_t *val, void *cd)
{
    ...
    case PhyConfig_AdvRemote:
        rv = _viper_ability_remote_get(pc, val);
        return rv;
#endif
    case PhyConfig_EEE:
        {
    ...
    case PhyConfig_RegisterAccess:
        addr = *val;
        rv = _viper_register_get(pc, addr, (uint32_t *)cd);
        return rv;
    default:
        break;
    }
