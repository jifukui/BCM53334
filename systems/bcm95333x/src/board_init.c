...
/*
     LED Re-Configuration Hook Function for Customer

     this function will be invoked after phy stage init
...
#define BCM54xxx_LED4_SEL(_pc)          0x6
#define BCM54xxx_LEDCTRL(_pc)           0x8
#define BCM54xxx_LEDSELECT(_pc)         0x8
...
int board_phy_init_callback(phy_ctrl_t *pc) {
    ...
    uint32_t val;
    int ioerr = 0;
    uint8   phy_led1_mode = BCM54xxx_LED1_SEL(pc);
    uint8   phy_led2_mode = BCM54xxx_LED2_SEL(pc);
    uint8   phy_led3_mode = BCM54xxx_LED3_SEL(pc);
    uint8   phy_led4_mode = BCM54xxx_LED4_SEL(pc);
    uint16   phy_led_ctrl = BCM54xxx_LEDCTRL(pc);
    uint16   phy_led_select = BCM54xxx_LEDSELECT(pc);
...
bcm5333x_fp_init(void)
{
...
#if (CFG_UIP_STACK_ENABLED || CFG_UIP_IPV6_ENABLED)
     ...
     */
    uint32 policy_entry[8] = {0x00000000, 0x02000000, 0x00000000, 0x20000110,
                              0x80004000, 0x04000000, 0x00000000, 0x00000020};

    uint32 redirection_entry[2] = {BCM5333x_ALL_PORTS_MASK, 0x0};
    ...
    bcm5333x_reg_set(0, R_FP_SLICE_ENABLE, 0xffff);
    ...
    bcm5333x_mem_set(0, M_FP_SLICE_MAP, slice_map_entry, 2);
    ...
    /* Program DUT'S MAC  : entry 24 of slice 1 */
    ...
    bcm5333x_mem_set(0, M_FP_TCAM(SYS_MAC_IDX), xy_entry, 15);
    ...
    for (i = BCM5333X_PORT_MIN; i <= BCM5333X_PORT_MAX; i++) {
      ...
      /* Program IPV6 all notes multicast MAC  : entry 25 of slice 1
       * FP_TCAM :
     ...
     bcm5333x_mem_set(0, M_FP_TCAM(SYS_MAC_IDX+1), xy_entry, 15);
     bcm5333x_mem_set(0, M_FP_POLICY_TABLE(SYS_MAC_IDX+1), policy_entry, 8);
     bcm5333x_mem_set(0, M_FP_GLOBAL_MASK_TCAM(SYS_MAC_IDX+1), global_tcam_mask_entry, 3);

     /* Program for IPV6 multicast MAC  : entry 26 of slice 1
      * FP_TCAM :
      * DA is start from bit 106
      * MASK of DA is start from bit 316
      */
    ...
    bcm5333x_mem_set(0, M_FP_TCAM(SYS_MAC_IDX+2), xy_entry, 15);
    bcm5333x_mem_set(0, M_FP_POLICY_TABLE(SYS_MAC_IDX+2), policy_entry, 8);
    bcm5333x_mem_set(0, M_FP_GLOBAL_MASK_TCAM(SYS_MAC_IDX+2), global_tcam_mask_entry, 3);
#endif /* CFG_UIP_IPV6_ENABLED */

#if (CFG_UIP_STACK_ENABLED)
    /* Set redireciton profile for each port.
     * -Use port number as index.
     * -No need to exclude source port nor trunk ports
     */
    for (i = BCM5333X_PORT_MIN; i <= BCM5333X_PORT_MAX; i++) {
...
board_init(void)
{
...
#ifdef CFG_SWITCH_LAG_INCLUDED
    {
        ...
        bcm5333x_reg_get(0, R_HASH_CONTROL, &val32);
        val32 |= 0x6;
        bcm5333x_reg_set(0, R_HASH_CONTROL, val32);
