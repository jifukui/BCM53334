/*
 * $Id: board_init.c,v 1.36.4.1 Broadcom SDK $
 *
 ...
 *   None
 */
extern uint32_t iproc_config_armpll_v7(uint32_t mode);
...
board_early_init(void)
{
   void (*funcptr)(void);
   /* Get chip revision */
   bcm5343x_chip_revision(0, &hr3_sw_info.devid, &hr3_sw_info.revid); 
   switch (hr3_sw_info.devid) {
       case BCM53434_DEVICE_ID: 
           if (SAL_IS_UMPLUS() || SAL_IS_LOADER() || SAL_IS_ROMCODE() || SAL_IS_UMDUMB()) {
               /* Initialize UART using default clock */
               iproc_config_genpll(2);  // 100 MHz
               iproc_config_armpll_v7(125); // 125Mhz
           }
           hr3_sw_info.cpu_clock = 125 * 1000000;          
#if CFG_CONSOLE_ENABLED
       ...
       case BCM56162_DEVICE_ID:
           if (SAL_IS_UMPLUS() || SAL_IS_LOADER() || SAL_IS_ROMCODE() || SAL_IS_UMDUMB()) {
               iproc_config_genpll(1);  // 200 MHz
               iproc_config_armpll_v7(600); // 600Mhz             
           }
           hr3_sw_info.cpu_clock = 600 * 1000000;
#if CFG_CONSOLE_ENABLED           
           /* Initialize UART using default clock */    
           board_console_init(CFG_UART_BAUDRATE, BOARD_CCA_WF2_UART_CLOCK);
#endif
           break;   
       default :
           /* Initialize UART using default clock */
           if (SAL_IS_UMPLUS() || SAL_IS_LOADER() || SAL_IS_ROMCODE() || SAL_IS_UMDUMB()) {
               iproc_config_genpll(0);  // 400 MHz
               iproc_config_armpll_v7(1250); // 1250Mhz                        
           }
           hr3_sw_info.cpu_clock = 1250 * 1000000;
#if CFG_CONSOLE_ENABLED           
           board_console_init(CFG_UART_BAUDRATE, BOARD_CCA_HR3_UART_CLOCK);
#endif
           break;               
   }
   ...
   (*funcptr)();
   sal_timer_init(hr3_sw_info.cpu_clock, TRUE);
...
bcm5343x_fp_init(void)
{
    ...
    uint32 redirection_entry[2] = {BCM5343X_ALL_PORTS_MASK, 0x0};    
    ...
    bcm5343x_reg_set(0, R_FP_SLICE_ENABLE, 0xffff);
    ...
    for (i = BCM5343X_LPORT_MIN; i <= BCM5343X_LPORT_MAX; i++) {
        ...
        port_field_sel_entry[0] = port_field_sel_entry[0] & 0xffffe00f;
        port_field_sel_entry[0] = 0xb << 9;
        bcm5343x_mem_set(0, M_FP_PORT_FIELD_SEL(i), port_field_sel_entry, 6);
    }

    bcm5343x_mem_set(0, M_FP_SLICE_MAP, slice_map_entry, 2);
    ...
    tcam_entry[11] = 0xf0000000;
    tcam_entry[12] = 0xffffffff;
    tcam_entry[13] = 0x00000fff;

    bcm5343x_dm_to_xy(tcam_entry, xy_entry, 15, 480);
    
    bcm5343x_mem_set(0, M_FP_TCAM(SYS_MAC_IDX), xy_entry, 15);
    bcm5343x_mem_set(0, M_FP_POLICY_TABLE(SYS_MAC_IDX), policy_entry, 8);
    bcm5343x_mem_set(0, M_FP_GLOBAL_MASK_TCAM(SYS_MAC_IDX), global_tcam_mask_entry, 3);

    for (i = BCM5343X_LPORT_MIN; i <= BCM5343X_LPORT_MAX; i++) {
        bcm5343x_mem_get(0, M_FP_PORT_FIELD_SEL(i), port_field_sel_entry, 6);
        port_field_sel_entry[0] = (port_field_sel_entry[0] & 0xc0001ffff)
                                                           | 0x2ca00000;
        bcm5343x_mem_set(0, M_FP_PORT_FIELD_SEL(i), port_field_sel_entry, 6);
    }
    ...
    tcam_entry[6] = (mac_addr[0] >> 6) & 0x00000003;
     ...
     bcm5343x_mem_set(0, M_FP_TCAM(SYS_MAC_IDX+1), xy_entry, 15);
     bcm5343x_mem_set(0, M_FP_POLICY_TABLE(SYS_MAC_IDX+1), policy_entry, 8);
     bcm5343x_mem_set(0, M_FP_GLOBAL_MASK_TCAM(SYS_MAC_IDX+1), global_tcam_mask_entry, 3);
    ...
    bcm5343x_mem_set(0, M_FP_TCAM(SYS_MAC_IDX+2), xy_entry, 15);
    bcm5343x_mem_set(0, M_FP_POLICY_TABLE(SYS_MAC_IDX+2), policy_entry, 8);
    bcm5343x_mem_set(0, M_FP_GLOBAL_MASK_TCAM(SYS_MAC_IDX+2), global_tcam_mask_entry, 3);
...
board_init(void)
{
   ...
   nvram_init();
...
#ifdef CFG_SWITCH_LAG_INCLUDED
    {
        ...
        bcm5343x_reg_get(0, R_HASH_CONTROL, &val32);
        val32 |= 0x6;
        bcm5343x_reg_set(0, R_HASH_CONTROL, val32);
