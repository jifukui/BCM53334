/*
 * $Id: mdk_phy.c,v 1.21 Broadcom SDK $
 *
...
extern phy_bus_t phy_bus_bcm5343x_miim_int;
extern phy_bus_t phy_bus_bcm95343xk_miim_ext;

static phy_bus_t *bcm5343x_phy_bus[] = {
    &phy_bus_bcm5343x_miim_int,
    &phy_bus_bcm95343xk_miim_ext,
    NULL
};
...
#define PHY_CTRL_NUM_MAX  (BCM5343X_LPORT_MAX*2)
/* 
...
bmd_phy_probe_default(int unit, int lport, phy_driver_t **phy_drv)
{
    ...
    if ((pc && pc->next) || 
        (pc && !sal_strcmp(pc->drv->drv_name,"bcm56160"))) {
                         ...
                         uint32_t phy_reg, uint32_t *phy_data)
{
    ...
    rv = bcm5343x_mem_set(unit, block_id, addr, xlport_wc_ucmem_data, 4);

    if (CDK_SUCCESS(rv)) {
        rv = bcm5343x_mem_get(unit, block_id, addr, xlport_wc_ucmem_data, 4);            
    }
...
bmd_phy_attach(int unit, int lport)
{
    ...
    BMD_PORT_PHY_BUS(unit, lport) = bcm5343x_phy_bus;
...
bmd_phy_ability_set(int unit, int lport, char *name, int ability)
{
    ...
    while (pc != NULL) {
        if (name && pc->drv && pc->drv->drv_name &&
            (sal_strcmp(pc->drv->drv_name, "bcmi_qtce_xgxs") != 0)) {
            pc = pc->next;
            continue;
        }
...
bmd_phy_eee_set(int unit, int lport, int mode)
{
    ...
    if (pc) {
        ...
        rv = PHY_CONFIG_SET(pc, PhyConfig_EEE, eee_mode, NULL);
        if (mode == PHY_EEE_NONE && rv == CDK_E_UNAVAIL) {
            rv = CDK_E_NONE;
        }

        if (pc->next) {
            rv = PHY_CONFIG_SET(pc->next, PhyConfig_EEE, eee_mode, NULL);
        }
...
phy_pause_get(uint8 unit, uint8 lport, BOOL *tx_pause, BOOL *rx_pause)
{
    ...
    if (PHY_EXTERNAL_MODE(lport)) {
        ...
        if (!sal_strcmp(pc->drv->drv_name,"bcmi_qtce_xgxs")) {
