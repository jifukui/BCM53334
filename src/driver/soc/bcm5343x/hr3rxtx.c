/*
 * $Id: hr3rxtx.c,v 1.6 Broadcom SDK $
 *
...
/* Forwards */
static void bcm5343x_rx_direct(void);
static void bcm5343x_rx_refill(void);
static void bcm5343x_rx_retrieve(soc_rx_packet_t *pkt);
static void bcm5343x_rxtx_task(void *data);
static sys_error_t bcm5343x_tx_internal(soc_tx_packet_t *pkt);




static void
bcm5343x_rx_direct(void)
{
...
APISTATIC void
bcm5343x_rx_retrieve(soc_rx_packet_t *pkt)
{
...
bcm5343x_rxtx_stop(void)
{
    ...
    bcm5343x_reg_get(0, R_EPC_LINK_BMAP_64, &val);
    val &= ~0x1;
    bcm5343x_reg_set(0, R_EPC_LINK_BMAP_64, val);
...
bcm5343x_rxtx_init(void)
{
    ...
    bcm5343x_reg_get(unit, R_EPC_LINK_BMAP_64, &val);
    val |= 0x1;
    bcm5343x_reg_set(unit, R_EPC_LINK_BMAP_64, val);

    bcm5343x_mem_get(unit, M_EGR_ENABLE(CPU_PORT_INDEX), &val, 1);
    val |= 0x1;
    bcm5343x_mem_set(unit, M_EGR_ENABLE(CPU_PORT_INDEX), &val, 1);
     ...
     */
    val = (0x1 << 6) | 32;
    bcm5343x_reg_set(unit, R_IP_TO_CMICM_CREDIT_TRANSFER, val);
    /*
    ...
    */
    ...
    task_add(bcm5343x_rxtx_task, (void *)NULL);
...
bcm5343x_tx_internal(soc_tx_packet_t *pkt)
{
    int i;
    uint32 port;
    ...
    if (pkt->port_bitmap != 0) {
        /* Use SOBMH mode. */
        tx_dcb->w1 = (1 << 19);
        for (port = BCM5343X_LPORT_MIN; port <= BCM5343X_LPORT_MAX; port++) {
            if (pkt->port_bitmap & (1 << port)) {
                break;
            }
        }

        if (port > BCM5343X_PORT_MAX) {
...
sys_error_t
bcm5343x_tx(uint8 unit, soc_tx_packet_t *pkt)
{
...
sys_error_t
bcm5343x_rx_set_handler(uint8 unit, SOC_RX_HANDLER fn, BOOL intr)
{
    if (fn == NULL || unit > 0) {
...
sys_error_t
bcm5343x_rx_fill_buffer(uint8 unit, soc_rx_packet_t *pkt)
{
    uint8 i;
    uint32 val;
