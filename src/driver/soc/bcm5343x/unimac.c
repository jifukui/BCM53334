/*
 * $Id: unimac.c,v 1.5 Broadcom SDK $
 *
...
soc_egress_drain_cells(uint8 unit, uint8 lport, uint32 drain_timeout)
{
    ...
    bcm5343x_reg_get(unit, R_EGRMETERINGCONFIG(lport), &orig_val);
    val = 0;
    bcm5343x_reg_set(unit, R_EGRMETERINGCONFIG(lport), val);

    
    do {
        count = 0;
        for (cos = 0; cos < COS_QUEUE_NUM; cos++) {
            bcm5343x_reg_get(unit, R_COSLCCOUNT(cos, lport), &val);
            count += val;
        }
    } while (count != 0);

    bcm5343x_reg_set(unit, R_EGRMETERINGCONFIG(lport), orig_val);
}
...
_mac_uni_sw_reset(uint8 unit, uint8 lport, BOOL reset_assert)
{
    ...
    bcm5343x_reg_get(unit, SOC_PORT_BLOCK(lport), 
...
_mac_uni_drain_cells(uint8 unit, uint8 lport)
{
     ...
     */
    bcm5343x_reg_set(unit, R_MMUFLUSHCONTROL, (1 << lport));
    ...
    bcm5343x_reg_set(unit, R_MMUFLUSHCONTROL, 0);
}
...
mac_uni_enable_set(uint8 unit, uint8 lport, BOOL enable)
{
    ...
    if (!enable) {
        ...
        bcm5343x_reg_set(unit, R_EPC_LINK_BMAP_64, val & ~(1 << lport));
...
mac_uni_init(uint8 unit, uint8 lport)
{
    ...
    if (0 == SOC_PORT_BLOCK_INDEX(lport)) {
        bcm5343x_reg_set(unit, SOC_PORT_BLOCK(lport), R_GPORT_RSV_MASK, 0x70);
  	    bcm5343x_reg_set(unit, SOC_PORT_BLOCK(lport), R_GPORT_STAT_UPDATE_MASK, 0x70);    
    }
    ...
    bcm5343x_reg_set(unit, SOC_PORT_BLOCK(lport), R_MAC_0(SOC_PORT_BLOCK_INDEX(lport)), mac_0);
    bcm5343x_reg_set(unit, SOC_PORT_BLOCK(lport), R_MAC_1(SOC_PORT_BLOCK_INDEX(lport)), mac_1);    
