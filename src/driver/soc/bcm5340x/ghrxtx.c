/*
 * $Id: ghrxtx.c,v 1.10 Broadcom SDK $
 *
...
#define PCI_TO_PTR(a)   _PCI_TO_PTR(a)
#define PTR_TO_PCI(x)   _PTR_TO_PCI(x)
#ifdef CFG_CPU_ARM_CORTEX_A9_ENABLE
...
/* Forwards */
static void bcm5340x_rx_direct(void);
static void bcm5340x_rx_refill(void);
static void bcm5340x_rx_retrieve(soc_rx_packet_t *pkt);
static void bcm5340x_rxtx_task(void *data);
static sys_error_t bcm5340x_tx_internal(soc_tx_packet_t *pkt);



static void
bcm5340x_rx_direct(void)
{
...
APISTATIC void
bcm5340x_rx_retrieve(soc_rx_packet_t *pkt)
{
...
bcm5340x_rxtx_stop(void)
{
    ...
    bcm5340x_reg_get(0, R_EPC_LINK_BMAP_64, &val);
    val &= ~0x1;
    bcm5340x_reg_set(0, R_EPC_LINK_BMAP_64, val);
...
bcm5340x_rxtx_init(void)
{
    ...
    bcm5340x_reg_get(unit, R_EPC_LINK_BMAP_64, &val);
    val |= 0x1;
    bcm5340x_reg_set(unit, R_EPC_LINK_BMAP_64, val);

    bcm5340x_mem_get(unit, M_EGR_ENABLE(CPU_PORT_INDEX), &val, 1);
    val |= 0x1;
    bcm5340x_mem_set(unit, M_EGR_ENABLE(CPU_PORT_INDEX), &val, 1);
     ...
     */
    val = (0x1 << 6) | 32;
    bcm5340x_reg_set(unit, R_IP_TO_CMICM_CREDIT_TRANSFER, val);
   ...
   /*
      ...
      */   
    val = SYS_REG_READ32(R_CMICD_S0_IDM_IO_CONTROL_DIRECT);
    val |= (3);
    SYS_REG_WRITE32(R_CMICD_S0_IDM_IO_CONTROL_DIRECT,val);

    val = SYS_REG_READ32(R_CMICD_M0_IDM_IO_CONTROL_DIRECT);
    val |= ((1<<31)  | (1<<30) | (0xB<<16) | (0x7<<7) | (3));
    SYS_REG_WRITE32(R_CMICD_M0_IDM_IO_CONTROL_DIRECT,val);
    ...
    task_add(bcm5340x_rxtx_task, (void *)NULL);
...
bcm5340x_tx_internal(soc_tx_packet_t *pkt)
{
    int i;
    uint32 port;
    ...
    if (pkt->port_bitmap != 0) {
        /* Use SOBMH mode. */
        tx_dcb->w1 = (1 << 19);
        for (port = BCM5340X_LPORT_MIN; port <= BCM5340X_LPORT_MAX; port++) {
            if (pkt->port_bitmap & (1 << port)) {
                break;
            }
        }

        if (port > BCM5340X_PORT_MAX) {
        ...
        tx_dcb->w3 = (port & 0x7F);
        /* Use cosq 7 and enable UNICAST */
        tx_dcb->w4 = 0x40000 | (0x7 << 8);
...
sys_error_t
bcm5340x_tx(uint8 unit, soc_tx_packet_t *pkt)
{
...
sys_error_t
bcm5340x_rx_set_handler(uint8 unit, SOC_RX_HANDLER fn, BOOL intr)
{
    if (fn == NULL || unit > 0) {
...
sys_error_t
bcm5340x_rx_fill_buffer(uint8 unit, soc_rx_packet_t *pkt)
{
    uint8 i;
    uint32 val;
