/*
 * $Id: xlmac.c,v 1.14 Broadcom SDK $
 *
...
soc_port_credit_reset(uint8 unit, uint8 lport)
{
    ...
    if (SOC_PORT_BLOCK(lport) == XLPORT5_BLOCK_ID) {
        bcm5340x_reg_get(unit, SOC_PORT_BLOCK(lport), 
                     R_PGW_GX_TXFIFO_CTRL(SOC_PORT_BLOCK_INDEX(lport)), &txfifo_ctrl);
    } else {
        bcm5340x_reg_get(unit, SOC_PORT_BLOCK(lport), 
                     R_PGW_XL_TXFIFO_CTRL(SOC_PORT_BLOCK_INDEX(lport)), &txfifo_ctrl);
    }
    /* MAC_CLR_COUNT Bit[1] and CORE_CLR_COUNT Bit[0] */
    txfifo_ctrl |= 0x3;
    if (SOC_PORT_BLOCK(lport) == XLPORT5_BLOCK_ID) {
...
soc_egress_drain_cells(uint8 unit, uint8 lport, uint32 drain_timeout)
{
    ...
    bcm5340x_reg_get(unit, R_EGRMETERINGCONFIG(lport), &orig_val);
    val = 0;
    bcm5340x_reg_set(unit, R_EGRMETERINGCONFIG(lport), val);

    
    do {
        count = 0;
        for (cos = 0; cos < COS_QUEUE_NUM; cos++) {
            bcm5340x_reg_get(unit, R_COSLCCOUNT(cos, lport), &val);
            count += val;
        }
    } while (count != 0);

    bcm5340x_reg_set(unit, R_EGRMETERINGCONFIG(lport), orig_val);
}
...
_mac_xl_drain_cells(uint8 unit, uint8 lport)
{
    ...
    bcm5340x_reg64_get(unit, SOC_PORT_BLOCK(lport), 
    ...
    bcm5340x_reg_set(unit, R_MMUFLUSHCONTROL, (1 << lport));
     ...
     */
    do {
        bcm5340x_reg_get(unit, R_XQEMPTY(lport), &entry[0]);
    } while(entry[0] != 0xFF);
    ...
    /* Bring the port out of flush */
    bcm5340x_reg_set(unit, R_MMUFLUSHCONTROL, 0x0);
...
mac_xl_enable_set(uint8 unit, uint8 lport, BOOL enable)
{
    ...
    if (enable) {
        ...
        for (i = 0; i < 10; i++) {
            ...
            bcm5340x_mem_get(unit, M_EGR_PORT_REQUESTS(SOC_PORT_L2P_MAPPING(lport)), &val, 1);
            if (val != 0) {
                break;
            } else {
                sal_printf("retry %d\n", i);
        ...
        bcm5340x_reg_set(unit, R_EPC_LINK_BMAP_64, val | (1 << lport));
    } else {
        /* Disable RX */
        bcm5340x_reg64_set(unit, SOC_PORT_BLOCK(lport), 
            R_XLMAC_CTRL(SOC_PORT_BLOCK_INDEX(lport)), ctrl, 2);
...
mac_xl_init(uint8 unit, uint8 lport)
{
    ...
    entry[0] &= ~(0x7 << 4);
    /* 
     * Bit[6:4]: 4 = 10000, 3 = 2500, 2 = 1000, 1 = 100, 0 = 10 
