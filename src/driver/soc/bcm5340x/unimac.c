/*
 * $Id: unimac.c,v 1.9 Broadcom SDK $
 *
...
_mac_uni_drain_cells(uint8 unit, uint8 lport)
{
    ...
    bcm5340x_reg_set(unit, R_MMUFLUSHCONTROL, 0);
}
...
mac_uni_init(uint8 unit, uint8 lport)
{
    ...
    if (0 == SOC_PORT_BLOCK_INDEX(lport)) {
        bcm5340x_reg_set(unit, SOC_PORT_BLOCK(lport), R_GPORT_RSV_MASK, 0x70);
  	    bcm5340x_reg_set(unit, SOC_PORT_BLOCK(lport), R_GPORT_STAT_UPDATE_MASK, 0x70);    
    }
    ...
    bcm5340x_reg_set(unit, SOC_PORT_BLOCK(lport), R_MAC_0(SOC_PORT_BLOCK_INDEX(lport)), mac_0);
    bcm5340x_reg_set(unit, SOC_PORT_BLOCK(lport), R_MAC_1(SOC_PORT_BLOCK_INDEX(lport)), mac_1);    
